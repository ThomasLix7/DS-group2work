...
# Four Key Points in Calculating Salary
...
p4 <- ggplot(data, aes(x = Salary_Bin, fill = factor(Left))) +
  ...
  scale_x_discrete(breaks = selected_bins, labels = selected_labels)  # only show min, 25%, 75%, max
...
# Four Key Points in Calculating Balance
...
# Balance values for the four key scales selected (converted to characters to match ggplot)
...
#print(length(Balance_bins))
#print(length(Balance_labels))
...
p4 <- ggplot(data, aes(x = Balance_Bin, fill = factor(Left))) +
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + # Rotate x-axis labels to prevent overlap
  scale_x_discrete(breaks = Balance_bins, labels = Balance_labels)  # only show min, 25%, 50%, 75%, max
...
p1 <- ggplot(data, aes(x = Products_in_Use, fill = factor(Left))) +
  geom_bar(position = "stack") +  # 使用 geom_bar() 而不是 geom_histogram()
  scale_fill_manual(values = c("#AFD3E7", "#9A72C7"), labels = c("Retained", "Churned")) +
  theme_minimal() + 
...
```
```{r}
#print(nrow(data %>%filter(Products_in_Use == 1)))
#print(nrow(data %>%filter(Products_in_Use == 2 )))
#print(nrow(data %>%filter(Products_in_Use == 3 )))
#print(nrow(data %>%filter(Products_in_Use == 4 )))
#print('--------------')
#print(nrow(data %>%filter(Products_in_Use == 1 & Left == 0)))
#print(nrow(data %>%filter(Products_in_Use == 1 & Left == 1)))

#print(nrow(data %>%filter(Products_in_Use == 2 & Left == 0)))
#print(nrow(data %>%filter(Products_in_Use == 2 & Left == 1)))
#print(nrow(data %>%filter(Products_in_Use == 3 & Left == 0)))
#print(nrow(data %>%filter(Products_in_Use == 3 & Left == 1)))
#print(nrow(data %>%filter(Products_in_Use == 4 & Left == 0)))
#print(nrow(data %>%filter(Products_in_Use == 4 & Left == 1)))
